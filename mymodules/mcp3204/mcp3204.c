...
static unsigned int mcp3204_get_value(struct mcp3204_drvdata *data, int channel){
    ...
    data->tx[0] |= 1 << 1;  // SGL/DIFF
    data->tx[1] = c << 6;   // D2, D1, D0
    data->tx[2] = 0;
...
// DEVICE_ATTR -> create "struct device_attribute dev_attr_xxx"
//  1st arg -> name (dev_attr_xxx)
//  2nd arg -> permission
//  3rd arg -> callback for read
//  4th arg -> callback for write

static int mcp3204_probe(struct spi_device *spi){
    struct mcp3204_drvdata *data;
   int ret;

    printk(KERN_INFO "mcp3204_probe()\n");
...
static int mcp3204_remove(struct spi_device *spi){
    ...
    printk(KERN_INFO "mcp3204_remove\n");
    return 0;
}
...
static int mcp3204_init(void){
    ...
    if(!master){
        printk(KERN_ERR "i99spi_busnum_to_master returned NULL\n");
