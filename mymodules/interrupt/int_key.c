...
static irqreturn_t hello_key_isr(int irq, void *data){
...
static int my_probe(struct platform_device *pdev){
    ...
    // First method to get Linux IRQ number */
    ...
    dev_info(dev, "IRQ using platform_get_irq: %d\n", irq);

    ret = devm_request_irq(dev, irq, hello_key_isr, IRQF_TRIGGER_FALLING, HELLO_KEYS_NAME, dev);
    if(ret){
    ...
    if(ret != 0){
        dev_err(dev, "Counld not register misc driver\n");
        return ret;
    }
...
static int my_remove(struct platform_device *pdev){
    dev_info(&pdev->dev, "my_remove() is called.\n");
    misc_deregister(&helloworld_miscdevice);
    dev_info(&pdev->dev, "my_remove() is exited.\n");
    return 0;
}
...
MODULE_DESCRIPTION("This is a button interrupt platform driver");
